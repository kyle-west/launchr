...
const G = 0.5
const FUEL_COST = 5_252_437
const LAUNCH_COST = 70_345_456
let LAUNCH_ATTEMPTS = 0

function getPlayerXScalar() {
  const PLAYER_GOT_IT_IN_ONE_TRY = window.sessionStorage.playerMadeItInOneTry === 'true'
  if (!PLAYER_GOT_IT_IN_ONE_TRY) return 0.2 // consistent starting place (per browser, at least)
  const dist = clamp(0.05, Math.random(), 0.3)
  return Math.random() < 0.5 ? 1 - dist : dist 
}
...
function init () {
  ...
  const player = new Rocket(X_LIMIT_R * getPlayerXScalar(), Y_LIMIT) // right above the LAUNCH button
  
  function launch() {
    if (!player.inFlight && !document.body.classList.contains('overlayOpen')) {
      player.vel = getThrust()
      player.inFlight = true
      player.cost += LAUNCH_COST
      LAUNCH_ATTEMPTS++
      document.dispatchEvent(new LaunchEvent({ attempts: LAUNCH_ATTEMPTS }))
    }
  }
  function endLaunch(landed) {
    if (landed && player.inFlight) {
      console.log(`LANDED after ${LAUNCH_ATTEMPTS} attempt(s)`)
      document.dispatchEvent(new LandingEvent({ attempts: LAUNCH_ATTEMPTS, cost: player.cost }))
    }
  ...
  function render() {
    ...
    player.applyPhysics()
    if (player.inFlight) {
      player.cost += FUEL_COST
    }
  }

  return [render, player, () => cleanup.map(clean => clean()) ]
}

function runGame () {
  const [render, player, cleanup] = init()
  let currentFrame;
  
  let lastCostDOMUpdate;
  let lastCost = 0;
  const costDOM = document.getElementById('write-cost')

  function game() {
    render()
    if (lastCost !== player.cost){
      lastCost = player.cost
      clearTimeout(lastCostDOMUpdate)
      lastCostDOMUpdate = setTimeout(() => costDOM.innerHTML = player.cost.toLocaleString('en-US'), 0)
    }
